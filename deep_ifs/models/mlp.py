...
class ConvNet:
    def __init__(self, input_shape, target_size, nb_actions=1, encoding_dim=512,
        ...
        if chkpt_file is not None:
            ...
            self.chkpt_file = 'MLP.h5' if logger is None else (logger.path + 'MLP.h5')
        ...
        self.hidden = Dense(128, activation='relu')(self.hidden)
        self.hidden = Dense(128, activation='relu')(self.hidden)
        self.hidden = Dense(128, activation='relu')(self.hidden)
        ...
        # Models
        self.model = Model(outputs=[self.output], inputs=self.input)
