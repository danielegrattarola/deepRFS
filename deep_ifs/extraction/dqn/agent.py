  ...
  def play(self, num_games, idx):
    ...
    for i in xrange(num_games):
      ...
      while not terminal:
        ...
        s = self.mem.getState(-2)
        ss = self.mem.getState(-1)
        d.append([s, action, reward, ss, terminal])

    np.random.shuffle(np.array(d))
    np.save('sars_%s.npy' % i, d)
