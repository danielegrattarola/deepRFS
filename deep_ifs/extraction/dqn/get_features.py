...
from deepqnetwork_features import DeepQNetwork
...
for idx, f in enumerate(tqdm(files)):
	sars = np.load(f)
	for start in range(0, len(sars), 32):
		stop = start + 32
		if stop > len(sars):
			break
		if idx == 0:
			F = net.predict(pds_to_npa(sars[start:stop, 0]))
			A = pds_to_npa(sars[start:stop, 1])
			R = pds_to_npa(sars[start:stop, 2])
			FF = net.predict(pds_to_npa(sars[start:stop, 3]))
			DONE = pds_to_npa(sars[:, 4])
		else:
			new_F = net.predict(pds_to_npa(sars[start:stop, 0]))
			new_A = pds_to_npa(sars[start:stop, 1])
			new_R = pds_to_npa(sars[start:stop, 2])
			new_FF = net.predict(pds_to_npa(sars[start:stop, 3]))
