...
class DeepQNetwork:
    def __init__(self, num_actions, args):
        ...
        # create model
        layers = self._create_layers(num_actions)
        ...
        if args.target_steps:
            self.target_model = Model(layers=self._create_layers(num_actions))
            # Bug fix
    ...
    def _create_layers(self, num_actions):
        # create network
        ...
        layers.append(Conv((8, 8, 32), strides=4, init=init_xavier_conv,
                           activation=Rectlin(), batch_norm=self.batch_norm))
        # The second hidden layer convolves 64 filters of 4x4 with stride 2, again followed by a rectifier nonlinearity.
        layers.append(Conv((4, 4, 64), strides=2, init=init_xavier_conv,
                           activation=Rectlin(), batch_norm=self.batch_norm))
        # This is followed by a third convolutional layer that convolves 64 filters of 3x3 with stride 1 followed by a rectifier.
        layers.append(Conv((3, 3, 64), strides=1, init=init_xavier_conv,
                           activation=Rectlin(), batch_norm=self.batch_norm))
        # The final hidden layer is fully-connected and consists of 512 rectifier units.
        layers.append(
            Affine(nout=512, init=init_xavier_affine, activation=Rectlin(),
                   batch_norm=self.batch_norm))
        return layers

    def _set_input(self, states):
    ...
    def train(self, minibatch, epoch):
        ...
        assert prestates.shape[0] == actions.shape[0] == rewards.shape[0] == \
               poststates.shape[0] == terminals.shape[0]

        # feed-forward pass for poststates to get Q-values
        self._set_input(poststates)
        ...
        for i, action in enumerate(actions):
            if terminals[i]:
                ...
                targets[action, i] = float(rewards[i]) + self.discount_rate * \
                                                         maxpostq[0, i]
    ...
    def predict(self, states):
        ...
        assert states.shape == (
        (self.batch_size, self.history_length,) + self.screen_dim), states.shape

        # calculate Q-values for the states
        self._set_input(states)
